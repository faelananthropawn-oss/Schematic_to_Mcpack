...
// ----------------------------------------------------------------------------
// Fixes & Improvements
// - Now correctly handles `.schem` variants where the block data is wrapped under
...
// - Memory-optimization merging using Uint8Array visited flags.
// - Fixed more edge case bedrock translation issues including falling blocks
// - more performance and translation issues with extra blockstates deleting early
// ----------------------------------------------------------------------------
...
function buildPaletteArray(pal){
  ...
  // If sparse, count might be less than maxIndex+1, but that is ok just continue
...
return null; // unsupported encoding (shouldnt happen but who knows at this point)
}
...
const packNameArg = process.argv[3];

if (!filePath) {
  console.error("Usage: node commands.mjs <schem-file> \"Pack Name\"");
  process.exit(1);
}

try {
  const schem = await loadSchematic(filePath);
  const hasData = (schem.type === "classic")
    ? (schem.legacyBlocks && schem.legacyBlocks.length)
    : (schem.blocks && schem.blocks.length);

  if (!hasData) {
    console.error("No block data found — nothing to output.");
    process.exit(2);
  }

  const commandsFile = `${filePath}.setblock.txt`;
  await dumpSetblockCommands(schem, commandsFile);

  if (packNameArg) {
    await buildPack(commandsFile, packNameArg);
  }
